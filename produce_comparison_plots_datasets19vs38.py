...
sample_idx = [
    ...
    "RAD21",
    "SMC3",
]  # these modifiers present in both datasets
...
df38_sub_cell = pd.merge(df38, filtered_df38_chr6, left_on="Accession", right_on=filtered_df38_chr6.index, how="inner")[
    "Biosample term name"
].tolist()
...
df19_sub_cell = pd.merge(df19, filtered_df19_chr6, left_on="Accession", right_on=filtered_df19_chr6.index, how="inner")[
    "Biosample term name"
].tolist()


df38_labels_activity = pd.merge(df38, filtered_df38_chr6, left_on="Accession", right_on=filtered_df38_chr6.index, how="inner")[
    "Activity"
].tolist()
df38_labels_factor = pd.merge(df38, filtered_df38_chr6, left_on="Accession", right_on=filtered_df38_chr6.index, how="inner")[
    "Factor"
].tolist()
df38_labels_mark = pd.merge(df38, filtered_df38_chr6, left_on="Accession", right_on=filtered_df38_chr6.index, how="inner")[
    "Target"
].tolist()
df38_labels_cell = pd.merge(df38, filtered_df38_chr6, left_on="Accession", right_on=filtered_df38_chr6.index, how="inner")[
    "Biosample term name"
].tolist()

df19_labels_activity = pd.merge(df19, filtered_df19_chr6, left_on="Accession", right_on=filtered_df19_chr6.index, how="inner")[
    "Activity"
].tolist()
df19_labels_factor = pd.merge(df19, filtered_df19_chr6, left_on="Accession", right_on=filtered_df19_chr6.index, how="inner")[
    "Factor"
].tolist()
df19_labels_mark = pd.merge(df19, filtered_df19_chr6, left_on="Accession", right_on=filtered_df19_chr6.index, how="inner")[
    "Target"
].tolist()
df19_labels_cell = pd.merge(df19, filtered_df19_chr6, left_on="Accession", right_on=filtered_df19_chr6.index, how="inner")[
    "Biosample term name"
].tolist()
        ...
        for i, label1 in enumerate(unique_labels_in_cluster):
            for j, label2 in enumerate(unique_labels_in_cluster):
                if i < j:
                    ...
                    )  # /np.sqrt(count_label1*count_label2)
...
ax = sns.heatmap(
    co_occurrence_matrix, annot=False, cmap="coolwarm", cbar=True, square=True, xticklabels=unique_labels, yticklabels=unique_labels
)
...
som = MiniSom(som_x, som_y, co_occurrence_matrix.shape[1], sigma=1.0, learning_rate=0.5, random_seed=42)
...
for i in range(weights.shape[0]):
    for j in range(weights.shape[1]):
        ...
        hex = RegularPolygon(
            (xx[(i, j)], wy), numVertices=6, radius=0.95 / np.sqrt(3), facecolor=cm.Blues(umatrix[i, j]), alpha=0.4, edgecolor="gray"
        )
        ax.add_patch(hex)
...
for (i, j), labels in cell_labels.items():
    ...
    for idx, label in enumerate(labels):
        dy = -idx * line_height
        plt.text(wx, start_y + dy, label, fontsize=fontsize, ha="center", va="center", color="darkblue")
