...
#Read chromosome 6 dendrogram saved in Newick format where labels are integers and branch lengths reflect # of important genes
...
# Read the tree from Newick string and set parameters for visualization
s = modified_newick
t = Tree(s)

base_style = NodeStyle()
base_style["vt_line_color"] = "#000000"   # vertical branch segments
base_style["hz_line_color"] = "#000000"   # horizontal branch segments
base_style["vt_line_width"] = 25
base_style["hz_line_width"] = 25
base_style["vt_line_type"]  = 0           # solid
base_style["hz_line_type"]  = 0
base_style["size"] = 0                    
...
def layout(node):
    ...
    if not ancestor_with_piechart and node.dist >= N_high_importance_color and len(node.get_leaf_names()) >= N_samples:
        ...
        green_style["vt_line_type"] = 0 # solid
        green_style["hz_line_type"] = 0  # solid
        for desc in node.traverse():
        ...
        blue_style["vt_line_type"] = 0 # solid
        blue_style["hz_line_type"] = 0 # solid
        for desc in node.traverse():
...
#output_png = "./png_tree.pdf"
#t.render(output_png, tree_style=ts)
#  the PDF fill an 8.0" x 7.5" page at 600 dpi
t.render(
    "png_tree600.pdf",
    tree_style=ts,
    w=8.0, h=7.5, units="in",
    dpi=600
)

t.render(
    "patch_circle_plot_600.eps",
    tree_style=ts,
    w=8.0, h=7.5, units="in",
    dpi=600
)
...
plt.tight_layout()
legend_filename_png = "legend_solid_lines.png"
plt.savefig(legend_filename_png, format='png', dpi = 800)
plt.show()

#now run convert_to_png.py